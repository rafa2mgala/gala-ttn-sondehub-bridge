...
public final class Application {
    ...
    private SondehubUploader habUploader;
    private SondehubConfig sondeHubConfig;
    private Application(ApplicationConfig config) {
        this.config = config;
        this.sondeHubConfig  = new SondehubConfig();
        this.habUploader = SondehubUploader.create(this.sondeHubConfig);
    }
    ...
    private void messageReceived(LoraWanUplinkMessage message) {
        // TODO
         ...
         LoraWanUplinkMessage uplink = new LoraWanUplinkMessage(message.getNetwork(), time, "Galasat-Gala", 1, 1,
                 message.getPayloadRaw());
         for(String key: message.getPayloadFields().keySet()) {
        	 uplink.addField(key, message.getPayloadFields().get(key));
         }
         uplink.addGateway(message.getGateways().get(0).getId(), 37.699958413745186, -5.274089098541719, 58.0);
         this.habUploader.scheduleTelemetryUpload(Instant.now(), uplink);
    }
